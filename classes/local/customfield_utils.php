...
class customfield_utils {
    ...
    public static function get_sql_for_entity_customfields($type, $additionaljoins = [], $additionalfields = []) {
        if ($type === 'course') {
            ...
            $customfieldhandler = \local_activitylibrary\customfield\coursemodule_handler::create();
        } else {
            throw new \moodle_exception('wronghandlerforfilter', 'local_activitylibrary', '', (object) [
                'handlername' => $type,
                'filtername' => $type,
            ]);
        }
        ...
        foreach ($handler->get_fields() as $f) {
            if (!utils::is_field_hidden_filters($handler, $f->get('shortname'))) {
                ...
                if (!empty($joinsfields[$datafieldname])) {
                    throw new \moodle_exception('shortnameshouldbeunique', 'local_activitylibrary');
                }
