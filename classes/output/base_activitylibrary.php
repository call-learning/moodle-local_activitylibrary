...
/**
 * Base class for activity and course activitylibrary
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class base_activitylibrary implements renderable, templatable {
    ...
    public function __construct(
        ...
        $this->view = $view;
        $this->paging = $paging;
        $config = get_config('local_activitylibrary');
    ...
    public function get_preferences() {
        ...
        $sort = get_user_preferences('local_activitylibrary_user_sort_preference');
        if ($sort) {
            $sortparms = explode(',', $sort);
            $preferences['sortcolumn'] = $sortparms[0];
            $preferences['sortorder'] = $sortparms[1];
        }
        $view = get_user_preferences('local_activitylibrary_user_view_preference');
        if ($view && in_array($view, [self::VIEW_CARD, self::VIEW_LIST])) {
            $preferences['view'] = $view;
        }

        $paging = get_user_preferences('local_activitylibrary_user_paging_preference');
        if ($paging) {
    ...
    public function get_export_defaults($output, handler $handler) {
        $nocoursesurl = $output->image_url('noentities', 'local_activitylibrary')->out();
        $defaultvariables = [
    ...
    public function get_filters_content($handler) {
        ...
        $filterform = new filter_form(null, ['handler' => $handler],
            'post', '', ['class' => 'activitylibrary-filters-form p-0']);
